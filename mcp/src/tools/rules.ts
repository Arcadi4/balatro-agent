...
const inputSchema = z
  .object({
    response_format: z
      ...
      .default("markdown")
      .describe(
        "Output format. Use 'json' for programmatic parsing, 'markdown' for the prompt text.",
      ),
  })
  .strict();
...
export function registerRulesTool(server: McpServer, deps: Deps): void {
  server.registerTool(
    ...
    async (args) => {
      ...
      const envelope = formatResponse(structured, format, { toMarkdown: rulesToMarkdown });
      return { ...envelope };
    },
  );
}
